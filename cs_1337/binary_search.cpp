...
// prototype
tuple<int, int> binarySearch(int list[], int key, int low, int high,
                             int iterations) {
...
int main() {
  int list[] = {2,  4,  7,  10, 11, 28, 34, 45, 50,
                59, 60, 66, 69, 70, 75, 89, 95};
  ...
  if (index != -1) {
    cout << "It takes " << iterations << " iterations to find the key " << key
         << endl;
    ;
    cout << key << " is at index " << index << endl;
  } else {
    cout << "After " << iterations << " iterations " << key << " is not found!"
         << endl;
  }
}
